...
Class Action {
	...
	function save_class(){
		...
		$check = $this->db->query("SELECT * FROM class where ciclo = '$ciclo' ".(!empty($id) ? ' and id!=$id ' : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function save_curso(){
		...
		$check = $this->db->query("SELECT * FROM paralelos where curso ='$curso' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO paralelos set $data");
		}else{
			$save = $this->db->query("UPDATE paralelos set $data where id = $id");
		}
	...
	function delete_curso(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM paralelos where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function get_att_record(){
		extract($_POST);
		//$get = $this->db->query("SELECT s.* FROM students s inner join `class` c on c.id = s.class_id inner join class_subject cs on cs.class_id = c.id where cs.id = '$class_subject_id' ");
		$get = $this->db->query("SELECT s.* FROM students s inner join `paralelos` c on c.id = s.class_id inner join class_subject cs on cs.class_id = c.id where cs.id = '$class_subject_id' ");
		...
		$qry = $this->db->query("SELECT s.subject, f.name,c.curso, ci.ciclo FROM class_subject cs inner join paralelos c on c.id = cs.class_id inner join subjects s on s.id = cs.subject_id inner join class ci on ci.id = cs.ciclo_id inner join faculty f on f.id = cs.faculty_id where cs.id = {$class_subject_id} ");
